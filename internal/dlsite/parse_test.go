...
func TestParseWorkCensored(t *testing.T) {
	t.Parallel()
	c := codes.WorkCode("RJ01307318")
	f, err := os.Open(testdataPath("work", c))
	if err != nil {
	...
	if !errors.Is(err, errCensored) {
		t.Fatalf("Expected %q, got %q", errCensored, err)
	}
}
